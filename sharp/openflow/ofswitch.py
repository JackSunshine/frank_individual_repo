...
class ActiveBundleRunning(Exception):
    message = "Another active bundle 0x%(bundle_id)x is running"
...
class OpenFlowSwitchMixin(object):
    ...
    def install_packet_to_controller(self, table_id=0, priority=0,
                                     match=None, **match_kwargs):
        (_dp, ofp, _ofpp) = self._get_dp()
        self.install_output(port=ofp.OFPP_CONTROLLER,
                            table_id=table_id, priority=priority,
                            match=match, **match_kwargs)
